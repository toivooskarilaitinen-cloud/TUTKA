#!/usr/bin/env python3
"""Collect strictly exceptional news candidates from GDELT DOC 2.0."""
...
def fetch(category,query):
 ...
 return [{**item,"category":category} for item in payload.get("articles",[])]
...
def score(article,peers):
 ...
 corroborating={urlparse(p.get("url","")).netloc for p in peers if p is not article and similarity(title,p.get("title",""))>=.42}
...
def main():
 DATA.mkdir(exist_ok=True); HISTORY.mkdir(exist_ok=True); raw=[]; errors=[]
 for category,query in QUERIES.items():
  try: raw.extend(fetch(category,query))
  except Exception as exc: errors.append(f"{category}: {type(exc).__name__}")
 ...
 for item in raw:
  ...
  candidates.append({"category":item["category"],"type":kind(title),"score":value,"title":title,"question":question(item["category"]),"why":f"Otsikossa on poikkeavuussignaali ja havaintoa tukee {sources} toisistaan riippumatonta uutislähdettä tai historiallista avainsanaa.","source":urlparse(item.get("url","")).netloc.removeprefix("www."),"url":item.get("url",""),"age":"TUORE"})
 ...
 now=datetime.now(timezone.utc); date=now.date().isoformat(); snapshot={"date":date,"generated_at":now.isoformat(),"updated":now.strftime("%H:%M UTC"),"rejected":max(0,len(raw)-len(chosen)),"findings":chosen,"errors":errors,"method_version":"v0.1"}
